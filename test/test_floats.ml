...
let test_float2int () =
  ...
  let result = F32.float2int f32_42 Int8 ToZero ~signed:true in
  assert_true "F32 42.0 -> i8 signed" (result = Some (Z.of_int 42));

  let result = F32.float2int f32_42 Int8 ToZero ~signed:false in
  assert_true "F32 42.0 -> u8 unsigned" (result = Some (Z.of_int 42));

  (* Negative integer (signed) *)
  let f32_neg42 = F32.of_string "-42.0" in
  let result = F32.float2int f32_neg42 Int8 ToZero ~signed:true in
  assert_true "F32 -42.0 -> i8 signed" (result = Some (Z.of_int (-42)));

  (* Negative integer (unsigned) - should fail *)
  let result = F32.float2int f32_neg42 Int8 ToZero ~signed:false in
  assert_true "F32 -42.0 -> u8 unsigned fails" (result = None);

  (* Rounding tests *)
  let f32_2_7 = F32.of_string "2.7" in
  let result = F32.float2int f32_2_7 Int8 ToZero ~signed:true in
  assert_true "F32 2.7 ToZero -> 2" (result = Some (Z.of_int 2));

  let result = F32.float2int f32_2_7 Int8 Up ~signed:true in
  assert_true "F32 2.7 Up -> 3" (result = Some (Z.of_int 3));

  let result = F32.float2int f32_2_7 Int8 Down ~signed:true in
  assert_true "F32 2.7 Down -> 2" (result = Some (Z.of_int 2));

  let f32_neg2_7 = F32.of_string "-2.7" in
  let result = F32.float2int f32_neg2_7 Int8 ToZero ~signed:true in
  assert_true "F32 -2.7 ToZero -> -2" (result = Some (Z.of_int (-2)));

  let result = F32.float2int f32_neg2_7 Int8 Down ~signed:true in
  assert_true "F32 -2.7 Down -> -3" (result = Some (Z.of_int (-3)));

  (* Overflow tests *)
  let f32_200 = F32.of_string "200.0" in
  let result = F32.float2int f32_200 Int8 ToZero ~signed:true in
  assert_true "F32 200.0 -> i8 signed overflow" (result = None);

  let result = F32.float2int f32_200 Int8 ToZero ~signed:false in
  assert_true "F32 200.0 -> u8 unsigned ok" (result = Some (Z.of_int 200));

  let f32_300 = F32.of_string "300.0" in
  let result = F32.float2int f32_300 Int8 ToZero ~signed:false in
  assert_true "F32 300.0 -> u8 unsigned overflow" (result = None);

  (* NaN and Inf *)
  let f32_nan = F32.of_string "nan" in
  let f32_inf = F32.of_bits 0x7F800000l in
  let result = F32.float2int f32_nan Int32 ToZero ~signed:true in
  assert_true "F32 NaN -> int fails" (result = None);

  let result = F32.float2int f32_inf Int32 ToZero ~signed:true in
  assert_true "F32 Inf -> int fails" (result = None);

  (* Larger integer sizes *)
  let f32_big = F32.of_string "100000.0" in
  let result = F32.float2int f32_big Int32 ToZero ~signed:true in
  assert_true "F32 100000.0 -> i32" (result = Some (Z.of_int 100000));

  (* Test F16 float2int *)
  print_endline "  F16 float2int:";
  let f16_10 = F16.of_string "10.0" in
  let result = F16.float2int f16_10 Int8 ToZero ~signed:true in
  assert_true "F16 10.0 -> i8" (result = Some (Z.of_int 10));

  (* Test F64 float2int *)
  print_endline "  F64 float2int:";
  let f64_1000000 = F64.of_string "1000000.0" in
  let result = F64.float2int f64_1000000 Int32 ToZero ~signed:true in
  ...
  let result = F128.float2int f128_999 Int16 ToZero ~signed:true in
...
let test_int2float () =
  ...
  (* Simple positive integer *)
  let f32 = F32.int2float (Z.of_int 42) Int8 NearestEven ~signed:true in
  ...
  let f32_signed = F32.int2float (Z.of_int (-1)) Int8 NearestEven ~signed:true in
  assert_float_eq "F32 i8 -1 signed" (-1.0) (F32.to_float f32_signed);

  (* Larger integers *)
  let f32 = F32.int2float (Z.of_int 1000000) Int32 NearestEven ~signed:true in
  assert_float_eq "F32 i32 1000000" 1000000.0 (F32.to_float f32);

  (* Test F16 int2float *)
  print_endline "  F16 int2float:";
  let f16 = F16.int2float (Z.of_int 100) Int8 NearestEven ~signed:false in
  assert_float_eq ~eps:epsilon_f16 "F16 u8 100" 100.0 (F16.to_float f16);

  (* Test F64 int2float *)
  print_endline "  F64 int2float:";
  let f64 = F64.int2float (Z.of_int 123456789) Int32 NearestEven ~signed:true in
  assert_float_eq "F64 i32 123456789" 123456789.0 (F64.to_float f64);

  (* Test F128 int2float *)
  print_endline "  F128 int2float:";
  let f128 = F128.int2float (Z.of_int 999999) Int32 NearestEven ~signed:true in
  assert_float_eq "F128 i32 999999" 999999.0 (F128.to_float f128);

  (* Test 64-bit integers *)
  print_endline "  64-bit integers:";
  let big_int = Z.of_string "9223372036854775807" in  (* max int64 *)
  let f64 = F64.int2float big_int Int64 NearestEven ~signed:true in
  ...
  let unsigned_big = Z.of_string "18446744073709551615" in  (* max uint64 *)
  let f64 = F64.int2float unsigned_big Int64 NearestEven ~signed:false in
...
let test_float_int_roundtrip () =
  ...
  let test_roundtrip_f32 n =
    let z = Z.of_int n in
    let f = F32.int2float z Int32 NearestEven ~signed:true in
    match F32.float2int f Int32 ToZero ~signed:true with
    | Some z' -> Z.equal z z'
    | None -> false
  in
  ...
  let test_roundtrip_f64 n =
    let z = Z.of_int n in
    let f = F64.int2float z Int64 NearestEven ~signed:true in
    match F64.float2int f Int64 ToZero ~signed:true with
    | Some z' -> Z.equal z z'
    | None -> false
  in
...
let test_f16_edge_cases () =
  ...
  let min_denorm = F16.of_bits 0x0001 in  (* smallest denormal *)
  let max_denorm = F16.of_bits 0x03FF in  (* largest denormal *)
  let min_normal = F16.of_bits 0x0400 in  (* smallest normal *)
  ...
  assert_int_eq "F16 max_denorm + min_denorm = min_normal" 0x0400 (F16.to_bits sum);
  ...
  assert_fpclass_eq "F16 min_normal/2 is subnormal" FP_subnormal (F16.fpclass almost_denorm);
  ...
  let one = F16.of_bits 0x3C00 in
  let eps16 = F16.of_bits 0x1400 in  (* 2^-10 = machine epsilon for F16 *)
  ...
  let half_eps = F16.of_bits 0x1000 in  (* 2^-11 *)
  let one_plus_half_eps = F16.add one half_eps in
  (* Should round to 1.0 (ties to even, 1.0 has even mantissa) *)
  assert_int_eq "F16 1 + eps/2 rounds to 1" 0x3C00 (F16.to_bits one_plus_half_eps);

  (* Overflow at boundary *)
  print_endline "  Overflow boundaries:";
  let max_normal = F16.of_bits 0x7BFF in  (* 65504 *)
  ...
  assert_true "F16 max + 1 no overflow" (not (is_inf (F16.to_float no_overflow)));

  (* Negative overflow *)
  let neg_max = F16.of_bits 0xFBFF in  (* -65504 *)
  ...
  let snan = F16.of_bits 0x7C01 in  (* signaling NaN *)
  let qnan = F16.of_bits 0x7E00 in  (* quiet NaN *)
...
let test_f32_edge_cases () =
  ...
  let f32_min_normal = F32.of_bits 0x00800000l in  (* 2^-126 *)
  let f32_max_normal = F32.of_bits 0x7F7FFFFFl in  (* (2 - 2^-23) * 2^127 *)
  let f32_min_denorm = F32.of_bits 0x00000001l in  (* 2^-149 *)
  let f32_max_denorm = F32.of_bits 0x007FFFFFl in

  assert_fpclass_eq "F32 min_normal class" FP_normal (F32.fpclass f32_min_normal);
  assert_fpclass_eq "F32 max_normal class" FP_normal (F32.fpclass f32_max_normal);
  assert_fpclass_eq "F32 min_denorm class" FP_subnormal (F32.fpclass f32_min_denorm);
  assert_fpclass_eq "F32 max_denorm class" FP_subnormal (F32.fpclass f32_max_denorm);
  ...
  assert_int32_eq "F32 +0 + -0 = +0" 0x00000000l (F32.to_bits (F32.add pos_zero neg_zero));
  assert_int32_eq "F32 -0 + -0 = -0" 0x80000000l (F32.to_bits (F32.add neg_zero neg_zero));
  assert_int32_eq "F32 1 * -0 = -0" 0x80000000l (F32.to_bits (F32.mul one neg_zero));
  assert_int32_eq "F32 -1 * -0 = +0" 0x00000000l (F32.to_bits (F32.mul neg_one neg_zero));

  (* Division by zero *)
  print_endline "  Division by zero:";
  assert_int32_eq "F32 1/+0 = +inf" 0x7F800000l (F32.to_bits (F32.div one pos_zero));
  assert_int32_eq "F32 1/-0 = -inf" 0xFF800000l (F32.to_bits (F32.div one neg_zero));
  assert_int32_eq "F32 -1/+0 = -inf" 0xFF800000l (F32.to_bits (F32.div neg_one pos_zero));
  assert_int32_eq "F32 -1/-0 = +inf" 0x7F800000l (F32.to_bits (F32.div neg_one neg_zero));
  ...
  assert_int32_eq "F32 inf + -inf = NaN"
    ...
    (F32.to_bits (F32.add inf neg_inf));  (* Just check it's NaN *)
  assert_true "F32 inf + -inf is NaN" (is_nan (F32.to_float (F32.add inf neg_inf)));

  (* Precision edge cases *)
  print_endline "  Precision:";
  let f32_eps = F32.of_bits 0x34000000l in  (* 2^-23, machine epsilon *)
  ...
  assert_float_eq "F32 -5 rem 3 = -2" (-2.0) (F32.to_float (F32.rem (F32.of_string "-5.0") three));
...
let test_f64_edge_cases () =
  ...
  print_endline "  IEEE 754 constants:";
  let f64_min_normal = F64.of_bits 0x0010000000000000L in  (* 2^-1022 *)
  let f64_max_normal = F64.of_bits 0x7FEFFFFFFFFFFFFFL in
  let f64_min_denorm = F64.of_bits 0x0000000000000001L in  (* 2^-1074 *)

  assert_fpclass_eq "F64 min_normal class" FP_normal (F64.fpclass f64_min_normal);
  assert_fpclass_eq "F64 max_normal class" FP_normal (F64.fpclass f64_max_normal);
  assert_fpclass_eq "F64 min_denorm class" FP_subnormal (F64.fpclass f64_min_denorm);
  ...
  assert_int64_eq "F64 +0 + -0 = +0" 0x0000000000000000L (F64.to_bits (F64.add pos_zero neg_zero));
  assert_int64_eq "F64 -0 + -0 = -0" 0x8000000000000000L (F64.to_bits (F64.add neg_zero neg_zero));
  assert_int64_eq "F64 1 * -0 = -0" 0x8000000000000000L (F64.to_bits (F64.mul one neg_zero));
  assert_int64_eq "F64 -1 * -0 = +0" 0x0000000000000000L (F64.to_bits (F64.mul neg_one neg_zero));
  ...
  print_endline "  Precision:";
  let f64_eps = F64.of_bits 0x3CB0000000000000L in  (* 2^-52, machine epsilon *)
  ...
  let large_int = F64.of_string "9007199254740992.0" in  (* 2^53, largest exact integer *)
  let large_int_plus_one = F64.add large_int one in
  (* 2^53 + 1 should round to 2^53 due to precision limits *)
  assert_float_eq "F64 2^53 + 1 precision" 9007199254740992.0 (F64.to_float large_int_plus_one)
...
let test_f128_edge_cases () =
  ...
  (* NaN propagation *)
  assert_true "F128 NaN + 1 = NaN" (is_nan (F128.to_float (F128.add nan_val one)));
  assert_true "F128 NaN * 0 = NaN" (is_nan (F128.to_float (F128.mul nan_val zero)));
  ...
  assert_true "F128 1e308 * 10 is finite or inf"
    (let v = F128.to_float bigger in is_inf v || v > 0.0);
...
let test_float2int_edge_cases () =
  ...
  assert_true "F32 127 -> i8 ok" (F32.float2int f32_127 Int8 ToZero ~signed:true = Some (Z.of_int 127));
  assert_true "F32 128 -> i8 overflow" (F32.float2int f32_128 Int8 ToZero ~signed:true = None);
  assert_true "F32 -128 -> i8 ok" (F32.float2int f32_neg128 Int8 ToZero ~signed:true = Some (Z.of_int (-128)));
  assert_true "F32 -129 -> i8 overflow" (F32.float2int f32_neg129 Int8 ToZero ~signed:true = None);
  assert_true "F32 255 -> u8 ok" (F32.float2int f32_255 Int8 ToZero ~signed:false = Some (Z.of_int 255));
  assert_true "F32 256 -> u8 overflow" (F32.float2int f32_256 Int8 ToZero ~signed:false = None);
  ...
  assert_true "F32 32767 -> i16 ok" (F32.float2int f32_32767 Int16 ToZero ~signed:true = Some (Z.of_int 32767));
  assert_true "F32 32768 -> i16 overflow" (F32.float2int f32_32768 Int16 ToZero ~signed:true = None);
  assert_true "F32 -32768 -> i16 ok" (F32.float2int f32_neg32768 Int16 ToZero ~signed:true = Some (Z.of_int (-32768)));
  assert_true "F32 -32769 -> i16 overflow" (F32.float2int f32_neg32769 Int16 ToZero ~signed:true = None);
  assert_true "F32 65535 -> u16 ok" (F32.float2int f32_65535 Int16 ToZero ~signed:false = Some (Z.of_int 65535));
  assert_true "F32 65536 -> u16 overflow" (F32.float2int f32_65536 Int16 ToZero ~signed:false = None);
  ...
  assert_true "F64 max_i32 -> i32 ok" (F64.float2int f64_max_i32 Int32 ToZero ~signed:true = Some (Z.of_string "2147483647"));
  assert_true "F64 max_i32+1 -> i32 overflow" (F64.float2int f64_over_i32 Int32 ToZero ~signed:true = None);
  assert_true "F64 min_i32 -> i32 ok" (F64.float2int f64_min_i32 Int32 ToZero ~signed:true = Some (Z.of_string "-2147483648"));
  assert_true "F64 min_i32-1 -> i32 overflow" (F64.float2int f64_under_i32 Int32 ToZero ~signed:true = None);
  assert_true "F64 max_u32 -> u32 ok" (F64.float2int f64_max_u32 Int32 ToZero ~signed:false = Some (Z.of_string "4294967295"));
  assert_true "F64 max_u32+1 -> u32 overflow" (F64.float2int f64_over_u32 Int32 ToZero ~signed:false = None);
  ...
  (* 127.5 ToZero -> 127 (ok), Up -> 128 (overflow) *)
  assert_true "F32 127.5 ToZero -> i8 ok" (F32.float2int f32_127_5 Int8 ToZero ~signed:true = Some (Z.of_int 127));
  assert_true "F32 127.5 Up -> i8 overflow" (F32.float2int f32_127_5 Int8 Up ~signed:true = None);

  (* -128.5 ToZero -> -128 (ok), Down -> -129 (overflow) *)
  assert_true "F32 -128.5 ToZero -> i8 ok" (F32.float2int f32_neg128_5 Int8 ToZero ~signed:true = Some (Z.of_int (-128)));
  assert_true "F32 -128.5 Down -> i8 overflow" (F32.float2int f32_neg128_5 Int8 Down ~signed:true = None);

  (* Zero handling *)
  print_endline "  Zero handling:";
  let f32_pos_zero = F32.of_bits 0x00000000l in
  let f32_neg_zero = F32.of_bits 0x80000000l in
  assert_true "F32 +0 -> int = 0" (F32.float2int f32_pos_zero Int32 ToZero ~signed:true = Some Z.zero);
  assert_true "F32 -0 -> int = 0" (F32.float2int f32_neg_zero Int32 ToZero ~signed:true = Some Z.zero);

  (* Very small values that round to zero *)
  let f32_tiny = F32.of_string "0.1" in
  assert_true "F32 0.1 ToZero -> 0" (F32.float2int f32_tiny Int8 ToZero ~signed:true = Some Z.zero);
  assert_true "F32 0.1 Up -> 1" (F32.float2int f32_tiny Int8 Up ~signed:true = Some Z.one);
  assert_true "F32 0.1 Down -> 0" (F32.float2int f32_tiny Int8 Down ~signed:true = Some Z.zero);

  let f32_neg_tiny = F32.of_string "-0.1" in
  assert_true "F32 -0.1 ToZero -> 0" (F32.float2int f32_neg_tiny Int8 ToZero ~signed:true = Some Z.zero);
  assert_true "F32 -0.1 Up -> 0" (F32.float2int f32_neg_tiny Int8 Up ~signed:true = Some Z.zero);
  assert_true "F32 -0.1 Down -> -1" (F32.float2int f32_neg_tiny Int8 Down ~signed:true = Some (Z.of_int (-1)));

  (* NearestEven rounding *)
  print_endline "  NearestEven rounding:";
  ...
  assert_true "F32 0.5 NearestEven -> 0" (F32.float2int f32_0_5 Int8 NearestEven ~signed:true = Some Z.zero);
  assert_true "F32 1.5 NearestEven -> 2" (F32.float2int f32_1_5 Int8 NearestEven ~signed:true = Some (Z.of_int 2));
  assert_true "F32 2.5 NearestEven -> 2" (F32.float2int f32_2_5 Int8 NearestEven ~signed:true = Some (Z.of_int 2));
  assert_true "F32 -0.5 NearestEven -> 0" (F32.float2int f32_neg_0_5 Int8 NearestEven ~signed:true = Some Z.zero);
  assert_true "F32 -1.5 NearestEven -> -2" (F32.float2int f32_neg_1_5 Int8 NearestEven ~signed:true = Some (Z.of_int (-2)));

  (* NearestAway rounding *)
  print_endline "  NearestAway rounding:";
  assert_true "F32 0.5 NearestAway -> 1" (F32.float2int f32_0_5 Int8 NearestAway ~signed:true = Some Z.one);
  assert_true "F32 -0.5 NearestAway -> -1" (F32.float2int f32_neg_0_5 Int8 NearestAway ~signed:true = Some (Z.of_int (-1)))
...
let test_int2float_edge_cases () =
  ...
  assert_float_eq "i8 127 -> F32" 127.0 (F32.to_float (F32.int2float z_127 Int8 NearestEven ~signed:true));
  assert_float_eq "i8 -128 -> F32" (-128.0) (F32.to_float (F32.int2float z_neg128 Int8 NearestEven ~signed:true));
  assert_float_eq "u8 255 -> F32" 255.0 (F32.to_float (F32.int2float z_255 Int8 NearestEven ~signed:false));
  ...
  assert_float_eq "i16 32767 -> F32" 32767.0 (F32.to_float (F32.int2float z_32767 Int16 NearestEven ~signed:true));
  assert_float_eq "i16 -32768 -> F32" (-32768.0) (F32.to_float (F32.int2float z_neg32768 Int16 NearestEven ~signed:true));
  assert_float_eq "u16 65535 -> F32" 65535.0 (F32.to_float (F32.int2float z_65535 Int16 NearestEven ~signed:false));
  ...
  assert_float_eq "i32 max -> F64" 2147483647.0 (F64.to_float (F64.int2float z_max_i32 Int32 NearestEven ~signed:true));
  assert_float_eq "i32 min -> F64" (-2147483648.0) (F64.to_float (F64.int2float z_min_i32 Int32 NearestEven ~signed:true));
  assert_float_eq "u32 max -> F64" 4294967295.0 (F64.to_float (F64.int2float z_max_u32 Int32 NearestEven ~signed:false));

  (* Zero values *)
  print_endline "  Zero:";
  assert_float_eq "i8 0 -> F32" 0.0 (F32.to_float (F32.int2float Z.zero Int8 NearestEven ~signed:true));
  assert_float_eq "u8 0 -> F32" 0.0 (F32.to_float (F32.int2float Z.zero Int8 NearestEven ~signed:false));

  (* Large values that lose precision in F32 *)
  print_endline "  Precision loss in F32:";
  let z_large = Z.of_int 16777217 in  (* 2^24 + 1, not exactly representable in F32 *)
  let f32_large = F32.int2float z_large Int32 NearestEven ~signed:true in
  (* Should round to 16777216 or 16777218 *)
  assert_true "i32 2^24+1 -> F32 rounds"
    (let v = F32.to_float f32_large in v = 16777216.0 || v = 16777218.0);

  (* Same value in F64 is exact *)
  let f64_large = F64.int2float z_large Int32 NearestEven ~signed:true in
  ...
  let z_pow2 = Z.of_int 1024 in  (* 2^10 *)
  assert_float_eq "i16 1024 -> F16" 1024.0 (F16.to_float (F16.int2float z_pow2 Int16 NearestEven ~signed:true))
...
let test_rounding_edge_cases () =
  ...
  List.iter (fun (name, v) ->
    ...
    (* ToZero *)
    let r_tz = F32.to_float (F32.round ToZero f32) in
    assert_float_eq (Printf.sprintf "F32 round ToZero %s" name) (Float.trunc v) r_tz;

    (* Up *)
    let r_up = F32.to_float (F32.round Up f32) in
    assert_float_eq (Printf.sprintf "F32 round Up %s" name) (Float.ceil v) r_up;

    (* Down *)
    let r_dn = F32.to_float (F32.round Down f32) in
    assert_float_eq (Printf.sprintf "F32 round Down %s" name) (Float.floor v) r_dn;

    (* NearestAway *)
    let r_na = F32.to_float (F32.round NearestAway f32) in
    assert_true (Printf.sprintf "F32 round NearestAway %s is int" name)
  ...
  assert_true "F32 round inf = inf" (is_inf (F32.to_float (F32.round NearestEven inf32)));
  assert_true "F32 round -inf = -inf" (is_inf (F32.to_float (F32.round NearestEven neg_inf32)));
  assert_true "F32 round NaN = NaN" (is_nan (F32.to_float (F32.round NearestEven nan32)));

  (* Rounding zeros should preserve sign *)
  assert_int32_eq "F32 round +0 = +0" 0x00000000l (F32.to_bits (F32.round NearestEven pos_zero32));
  assert_int32_eq "F32 round -0 = -0" 0x80000000l (F32.to_bits (F32.round NearestEven neg_zero32));

  (* Large values that are already integers *)
  print_endline "  Large integers:";
  let big32 = F32.of_string "16777216.0" in  (* 2^24, exactly representable *)
  assert_float_eq "F32 round 2^24 = 2^24" 16777216.0 (F32.to_float (F32.round NearestEven big32));

  (* Denormals *)
  print_endline "  Denormals:";
  let min_denorm32 = F32.of_bits 0x00000001l in
  let rounded_denorm = F32.round NearestEven min_denorm32 in
  assert_int32_eq "F32 round min_denorm = 0" 0x00000000l (F32.to_bits rounded_denorm);

  let neg_min_denorm32 = F32.of_bits 0x80000001l in
  let rounded_neg_denorm = F32.round NearestEven neg_min_denorm32 in
  assert_int32_eq "F32 round -min_denorm = -0" 0x80000000l (F32.to_bits rounded_neg_denorm)
...
let test_abs_edge_cases () =
  ...
  assert_int32_eq "F32 abs min_denorm" 0x00000001l (F32.to_bits (F32.abs min_denorm32));
  assert_int32_eq "F32 abs -min_denorm" 0x00000001l (F32.to_bits (F32.abs neg_min_denorm32));
  ...
  assert_int64_eq "F64 abs -0" 0x0000000000000000L (F64.to_bits (F64.abs neg_zero64));
  assert_int64_eq "F64 abs -inf" 0x7FF0000000000000L (F64.to_bits (F64.abs neg_inf64));
  ...
  assert_int_eq "F16 abs -min_denorm" 0x0001 (F16.to_bits (F16.abs neg_min_denorm16))
